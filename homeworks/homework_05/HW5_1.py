...
# def find_cars(cars, criteria):
def find_cars(cars: dict, criteria: tuple) -> list:
    min_year, min_engine, max_price = criteria
    filtered = []

    # Фільтруємо за критеріями дз
    for brand, data in cars.items():
        ...
        if year >= min_year and engine >= min_engine and price <= max_price:
            filtered.append((brand, data))

    # Сортуємо по ціні. Те що в (data[4])
    filtered.sort(key=lambda x: x[1][4])

    # Беремо перші 5 штучок
    return filtered[:5]


matches = find_cars(car_data, search_criteria)
for brand, data in matches:
    print(brand, data)